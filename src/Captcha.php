...
class Captcha
{
    ...
    {
        $key = $this->config['session_key'] . $id;
        ...
        if (strtoupper($code) == $_SESSION[$key]) {
            unset($_SESSION[$key]);
            return true;
        }
        return false;
    ...
    public function render($id = '')
    {
        if (!$this->config['imageW']) {
            $this->config['imageW'] = $this->config['length'] * $this->config['fontSize'] * 1.5 + $this->config['length'] * $this->config['fontSize'] / 2;
        }
        if (!$this->config['imageH']) {
            $this->config['imageH'] = $this->config['fontSize'] * 2.0;
        }
        $this->im = imagecreate($this->config['imageW'], $this->config['imageH']);
        // 设置背景
        ...
        // 验证码使用随机字体
        $ttfPath = __DIR__ . '/../assets/' . ($this->config['useZh'] ? 'zhttfs' : 'ttfs') . '/';
        ...
        if ($this->config['useZh']) {
            ...
            for ($i = 0; $i < $this->config['length']; $i++) {
                ...
                imagettftext($this->im, $this->config['fontSize'], mt_rand(-40, 40), $this->config['fontSize'] * ($i + 1) * 1.5, $this->config['fontSize'] + mt_rand(10, 20), $this->color, $this->config['fontttf'], $code[$i]);
            }
        } else {
            for ($i = 0; $i < $this->config['length']; $i++) {
