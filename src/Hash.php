...
class Hash
{
    ...
    /**
     * 构造
     * @param string $algo 要使用的哈希算法名称，例如："md5"，"sha256"，"haval160,4" 等
     * @param int $options 进行哈希运算的可选设置，目前仅支持一个选项：HASH_HMAC
     * @param string $key 当 options 参数为 HASH_HMAC 时， 使用此参数传入进行 HMAC 哈希运算时的共享密钥。
     */
    ...
    /**
     * 拷贝当前哈希运算上下文
     * @param bool $clone 如果$clone为true则返回的是克隆的Hash类而非哈希运算上下文
     * @return mixed
     */
    ...
    /**
     * 使用给定文件的内容生成哈希值
     * @param string $algo 要使用的哈希算法的名称，例如："md5"，"sha256"，"haval160,4" 等。
     ...
     * @return string
     */
    public function finalStr($raw_output = false)
    {
     ...
     * 从文件向活跃的哈希运算上下文中填充数据
     * @param string $filename 要进行哈希运算的文件路径，支持 fopen 封装器。
     * @param resource $scontext 由 stream_context_create() 函数返回的流上下文。
     * @return bool
     */
     ...
     * @param int $length 要从 handle 向活跃的哈希运算上下文中拷贝 的最大字符数。 -1表示全部返回
